...
> **Status:** the v1 migration is **done** (`14d59c4`, `9009931`); the **ACOE v2** follow-ups (DECISIONS.md **D15**) are **done** too — `CATEGORY_MAX` rebalanced to the v2 maxes (audio **15**, engagement **10**), `legacyDims()` + `dimensions` dropped and `RewardDimensions` deleted (D14), and hashtags read from `ContentConcept.execution` (`ExecutionMetadata`). Spec below retained for reference.
